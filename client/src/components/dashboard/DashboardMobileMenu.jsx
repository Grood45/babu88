...
const DashboardMobileMenu = ({ open, menuItems }) => {
  ...
  return (
    <>
      <div>
        <div
          className={`bg-[#222222] p-4 fixed left-0 right-0 z-40 duration-300 ${!open ? "md:ml-16" : "md:ml-64"
            }`}
        >
        ...
        <div
          ...
          className={`fixed inset-0 w-[70%] sm:w-1/2 h-screen overflow-y-auto backdrop-blur bg-black/40 z-50 md:hidden transform transition-transform duration-500 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
            }`}
        >
          ...
          <div className="text-white bg-[#222222] font-bold">
            {menuItems.map((item) => {
              ...
              return (
                <div
                  key={item?.name}
                  className={`py-1 flex flex-col items-start gap-2 border-b border-gray-700 duration-300 ${isActive ? "bg-red-600" : "hover:bg-red-600"
                    }`}
                >
                  <div
                    className={`px-4 py-2.5 flex items-center justify-between w-full ${item?.submenu?.length > 0 ? "cursor-pointer" : ""
                      }`}
                    onClick={() =>
                      ...
                      {item?.submenu?.map((submenuItem) => {
                        ...
                        return (
                          <div
                            ...
                            className={`pl-8 py-3 flex flex-col gap-2 ${isSubmenuActive ? "bg-red-500" : ""
                              }`}
                          >
                            <Link
                              to={submenuItem?.path}
